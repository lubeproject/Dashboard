...
const Visiting = () => {
  ...
  const fetchAndCalculateLocation = async (qrcode) => {
    ...
    try {
      ...
      const { latitude: userLat, longitude: userLon, userid, name, shopname, role } = userDetails;
      setUserLocation({ latitude: userLat, longitude: userLon });

      if (location) { // Ensure device location is available
    ...
    if (visitId && !checkedOut) {
      ...
      try {
        const { error } = await supabase
          .from('represent_visiting1')
          .update({ checkouttime: getCurrentTime(), lastupdatetime: new Date() })
